...
数据集管理界面为可选 Gradio 组件：先执行 `uv sync --extra admin`，再运行 `uv run python -m scripts.gradio_admin`。界面只允许通过现有 JSONL 校验器导入 `VERIFIED` 记录，草稿会被跳过；也支持导出规范化 `VERIFIED` JSONL 和校验报告。
