...
| Research | 多源检索与研究 | 是 | Hybrid RAG + SQL + BM25 + Vector + Web/内部库 | **部分完成**：PostgreSQL 元数据过滤、全文/trigram/词覆盖、embedding 写入/重建、pgvector 查询及可配置词法/向量融合已具备；OpenSearch BM25 已接入 Research，支持去重、过滤与降级；Web 未接入。 |
