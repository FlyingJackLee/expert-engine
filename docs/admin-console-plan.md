...
| 知识库管理 | 按知识域、专家 Profile、主题浏览文档摘要和向量统计 | 按类型、主题、来源检索并查看证据片段、文档生命周期操作 | 已接入统计与摘要浏览 |
| 向量管理 | 查看向量模型/维度统计、触发重建 | 分批进度、失败重试、模型切换检查 | 已有命令行重建，待接 UI |
| 黄金样本 | 运行 Benchmark、查看汇总指标 | 上传/校验评测 JSON、样本版本、单案例差异查看 | 已接入评测页 |
| 评测报告 | 展示 Expert 能力分数、知识准备度和待补知识域 | 趋势比较、模型/权重对比 | 已接入基础报告页 |
| 运行辅助 | 检查配置、查看跨入口 Graph 节点事件和任务结果 | 任务队列、告警、持久化运行指标 | 已接入节点事件 API 与 Gradio 自动刷新页 |
